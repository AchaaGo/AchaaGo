...
export function CustomerFlow() {
  ...
  useEffect(()=>{boot();return()=>{socket.current?.close();if(searchTimer.current)clearTimeout(searchTimer.current);staleGuard.start()}},[]);
  ...
  async function requote(nextLoaders:number){if(busy)return;setLoaders(nextLoaders);if(!dropoff)return;setError('');setQuote(null);setBusy(true);try{setQuote(await api('/quotes',{method:'POST',body:JSON.stringify({pickup,dropoff,loaders:nextLoaders})}))}catch(e){setError(customerError(e))}finally{setBusy(false)}}
  ...
  if(screen==='route')return <main className="phone customer-screen route-screen">
    <section className="route-search" aria-labelledby="route-title">
      <div className="route-heading"><button className="icon-btn" disabled={busy} onClick={()=>setScreen('home')} aria-label="Буцах"><ArrowLeft aria-hidden="true"/></button><div><h1 id="route-title" className="display">Хаашаа ачих вэ?</h1><small>Авах: Одоогийн байршил</small></div></div>
      <label className="sr-only" htmlFor="destination">Хүргэх хаяг</label>
      <div className="destination-field"><MapPin size={20} aria-hidden="true"/><input ref={searchInput} id="destination" value={search} disabled={busy} onChange={e=>searchPlaces(e.target.value)} onKeyDown={onSearchKeyDown} placeholder="Хүргэх хаягаа хайх" role="combobox" aria-expanded={places.length>0} aria-controls={places.length>0?'destination-listbox':undefined} aria-autocomplete="list" aria-describedby="search-feedback" aria-activedescendant={highlight>=0&&places[highlight]?`place-option-${highlight}`:undefined} autoComplete="off"/>{search&&<button className="clear-search" disabled={busy} aria-label="Хүргэх хаягийг арилгах" onClick={()=>{searchPlaces('');searchInput.current?.focus()}}><X size={18} aria-hidden="true"/></button>}</div>
      <p id="search-feedback" className="search-feedback" role="status">{placesLoading?copy.searching:busy?copy.quoteLoading:dropoff?copy.selectedAddress:searched&&!places.length&&!error?copy.noResults:search.trim().length>0&&search.trim().length<3?copy.searchShort:copy.searchHelp}</p>
    ...
    <section className="sheet" aria-labelledby="service-title">
      ...
      <div className="order-action">
        ...
        {dropoff&&!quote&&!busy&&<button className="btn btn-outline" onClick={()=>requote(loaders)}>{copy.retryQuote}</button>}
        ...
        <p id="order-help" className="order-help" role="status">{busy?(ordering?copy.orderLoading:copy.quoteLoading):!dropoff?copy.needsAddress:!quote?copy.needsQuote:'Захиалгын нийт үнэ дээр харагдаж байна.'}</p>
      </div>
    </section>
  </main>;
