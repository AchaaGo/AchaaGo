...
export function PublicTracking({token}:{token:string}){
 ...
 if(error)return <main className="phone" style={{display:'grid',placeItems:'center',padding:28,textAlign:'center'}}><div><h1 className="display">Холбоос идэвхгүй</h1><p className="muted">{error}</p></div></main>;
 if(!order)return <main className="phone" style={{display:'grid',placeItems:'center',background:'var(--ink)'}}><span className="spinner"/></main>;
 return <main className="phone"><GoogleMapView route truck={!!order.driver} pickup={order.pickup} dropoff={order.dropoff} driverLocation={order.driver?.location??null}/><section className="sheet"><div className="grab"/><span className="badge">{mn.statuses[order.status]||order.status}</span><h1 className="display" style={{fontSize:22}}>Захиалгын явц</h1>{order.driver&&<div className="card" style={{padding:14,display:'flex',alignItems:'center',gap:12,marginBottom:14}}><span style={{width:48,height:48,borderRadius:'50%',background:'var(--ink)',color:'var(--accent)',display:'grid',placeItems:'center'}}><Truck/></span><div style={{flex:1}}><b>{order.driver.name}</b><span className="muted" style={{display:'flex',alignItems:'center',gap:4}}><Star size={14} fill="var(--accent)"/>{order.driver.rating||'Шинэ'}</span></div><strong>{order.driver.plate_number}</strong></div>}<div style={{display:'grid',gap:12,padding:16,background:'var(--ground)',borderRadius:16}}><div><small className="muted">Авах</small><div>{order.pickup.address}</div></div><div><small className="muted">Хүргэх</small><div>{order.dropoff.address}</div></div><div style={{display:'flex',justifyContent:'space-between'}}><span>{order.service_name}</span><span>{order.distance_km} км</span></div></div><p className="muted" style={{fontSize:13,textAlign:'center'}}>Энэхүү холбоос нь утасны дугаар болон төлбөрийн мэдээлэл харуулахгүй.</p></section></main>
}
